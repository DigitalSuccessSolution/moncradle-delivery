...
export default function OrderDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const pickupCoords = order?.kitchenId?.location?.coordinates;
  const dropoffCoords = order?.deliveryAddress?.location?.coordinates;

  const hasValidRoute = pickupCoords?.length === 2 && dropoffCoords?.length === 2;
  const pickupLocation: [number, number] | null = hasValidRoute ? [pickupCoords[1], pickupCoords[0]] : null;
  const dropoffLocation: [number, number] | null = hasValidRoute ? [dropoffCoords[1], dropoffCoords[0]] : null;

  const openGoogleMaps = () => {
    if (hasValidRoute) {
      window.open(`https://www.google.com/maps/dir/?api=1&origin=${pickupLocation![0]},${pickupLocation![1]}&destination=${dropoffLocation![0]},${dropoffLocation![1]}`, '_blank');
    }
  };
  ...
  const handleCompleteDelivery = async () => {
    ...
    if (order.paymentMethod === 'cod' && order.paymentStatus !== 'paid' && !cashCollected) {
      Swal.fire({
        title: "Cash Collection Pending",
        text: `Please collect ₹${order.totalAmount} and check the confirmation box below.`,
        icon: "warning",
        confirmButtonColor: "#1E4E70"
      });
      return;
  ...
  return (
    <div className="w-full font-sans animate-fade-in-up">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-2.5">
            ...
            {order.paymentStatus === 'paid' ? (
              <span className="bg-emerald-50 text-emerald-700 border border-emerald-200/60 px-2.5 py-1 rounded-md text-[13px] font-medium flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5" />
                <span>{order.paymentMethod === 'cod' ? 'Cash Collected' : 'Paid Online'}</span>
              </span>
            ) : (
              <span className="bg-amber-50 text-amber-700 border border-amber-200/60 px-2.5 py-1 rounded-md text-[13px] font-medium flex items-center gap-1.5">
                <AlertCircle className="w-3.5 h-3.5" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        {/* Map View */}
        {hasValidRoute && pickupLocation && dropoffLocation && (
          <div className="bg-white rounded-lg border border-slate-200/80 p-4 sm:p-6 mb-6 overflow-hidden">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[14px] font-medium text-slate-500 uppercase tracking-wider flex items-center gap-2">
            ...
            <div className="rounded-xl overflow-hidden border border-slate-200 bg-slate-50 p-1">
              <DeliveryMap pickup={pickupLocation} dropoff={dropoffLocation} />
            </div>
          </div>
        )}
                ...
                {order.paymentMethod === 'cod' && order.paymentStatus !== 'paid' && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
                    <input
                      ...
                      className="mt-0.5 w-5 h-5 rounded border-amber-300 text-emerald-600 focus:ring-emerald-500 bg-white cursor-pointer"
                    />
                    <label htmlFor="cash-collected" className="text-[14px] font-medium text-amber-900 cursor-pointer select-none">
                      I confirm that I have collected <strong className="text-emerald-700">₹{order.totalAmount}</strong> from the customer.
                    </label>
                  </div>
                )}
